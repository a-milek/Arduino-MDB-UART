...
void Reader_Reader(unsigned char index) {
	...
	switch(MDB_BUFFER[1].data)
	{
		...
		// READER DISABLE
			...
			sprintf_FSTR(buff, "CD%d#DISABLE", index + 1);
			EXT_UART_Transmit(buff);
			Reader_State[index] = MDB_DISABLED;
			MDB_ACK();
			ReaderSetIdle(index);
		...
		// READER ENABLE
			...
			MDB_ACK();
			sprintf_FSTR(buff, "CD%d#ENABLE", index + 1);
			EXT_UART_Transmit(buff);
		...
		// READER CANCEL
			...
			sprintf_FSTR(buff, "CD%d#CANCEL", index + 1);
			EXT_UART_Transmit(buff);
			Reader_State[index] = MDB_ENABLED;
		break;
		case 3:
		// READER DATA ENTRY RESPONSE
			...
			MDB_ACK();
			sprintf_FSTR(buff, "CD%d#DER#", index + 1);
			EXT_UART_Transmit(buff);
	...
	switch(MDB_BUFFER[1].data)
	{
		...
		// Peripheral ID request
			...
			//return;
			{
				...
				unsigned char buff[16];
				sprintf_FSTR(buff, "CD%d#PERIPHID#", index + 1);
				DebugMessage(buff);
				...
				sprintf_FSTR(buff, "%s#", ReaderVMCIDData[index].ManufacturerCode);
				DebugMessage(buff);
				...
				sprintf_FSTR(buff, "%s#", ReaderVMCIDData[index].SerialNumber);
				DebugMessage(buff);
				...
				sprintf_FSTR(buff, "%s#", ReaderVMCIDData[index].ModelRevision);
				DebugMessage(buff);
...
void Reader_Vend(unsigned char index) {
	...
	switch(MDB_BUFFER[1].data)
	{
		...
		// VEND REQUEST
			{
				...
				sprintf_FSTR(buff, "CD%d#VENDREQ#", index + 1);
				DebugMessage(buff);
		...
		// VEND CANCEL
			{
				...
				sprintf_FSTR(buff, "CD%d#VENDCANCEL\r\n", index + 1);
				DebugMessage(buff);
				ReaderSetIdle(index);
			}
			break;
		...
		// VEND SUCCESS
			...
			MDB_ACK();
			sprintf_FSTR(&buff, "CD%d#VENDSUCCESS#%d\r\n", index + 1, itemnumber);
			DebugMessage(buff);
			ReaderSetIdle(index);
		...
		// VEND FAILURE
			{
				...
				sprintf_FSTR(buff, "CD%d#VENDFAIL\r\n", index + 1);
				DebugMessage(buff);
		...
		// SESSION COMPLETE
			{
				...
				sprintf_FSTR(buff, "CD%d#SCOMPLETE\r\n", index + 1);
				DebugMessage(buff);
		...
		// NEGATIVE VEND REQUEST
			{
				...
				//product.
				sprintf_FSTR(buff, "CD%d#NEGVENDREQ#\r\n", index + 1);
				DebugMessage(buff);
				//unsigned char buff[6 + ReaderSetupData[index].Decimal_Places];
				uint16_t amount = MDB_BUFFER[2].data << 8;
				amount |= MDB_BUFFER[3].data;
		...
		// Unknown Subcommand from VMC
		{
			sprintf_FSTR(buff, "CD%d#unsupported [VEND]\r\n", index + 1);
			DebugMessage(buff);
			ReaderSetIdle(index);
		}
		break;
...
void Reader_Poll(unsigned char index) {
	...
	switch(Reader_Poll_Reply[index])
	{
		case MDB_REPLY_ACK:
			...
			unsigned char buff[24];
			sprintf_FSTR(buff, "CD%d#JUSTRESET\r\n", index + 1);
			DebugMessage(buff);
			ReaderSetIdle(index);
		break;
